...
<cfif structKeyExists(URL, "goDynamo")>
	<cfscript>
	dynamoDB = CreateObject('component', 'devObjective2016.model.awsServiceFactory').init().createServiceObject('dynamoDB');
	...
	switch(lcase(trim(URL.goDynamo))){
		...
		case 'putItem':
			...
			itemMaker = CreateObject('component','devObjective2016.model.dynamoItemMaker').init();
